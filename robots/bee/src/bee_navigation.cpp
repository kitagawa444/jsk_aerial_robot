...
void BeeNavigator::initialize(ros::NodeHandle nh, ros::NodeHandle nhp,
                              ...
                              boost::shared_ptr<aerial_robot_estimation::StateEstimator> estimator, double loop_du)
{
  ...
  assembly_nav_sub_ = nh_.subscribe("/assembly/uav/nav", 1, &BeeNavigator::assemblyNavCallback, this);
  assembly_target_rot_sub_ =
      nh_.subscribe("/assembly/final_target_baselink_rot", 1, &BeeNavigator::setAssemblyFinalTargetBaselinkRotCallback, this);
  bee_robot_model_ = boost::dynamic_pointer_cast<BeeRobotModel>(robot_model);

  for (int i = 0; i < max_modules_num_; i++)
  {
    std::string module_name = string("/") + getMyName() + std::to_string(i + 1);
    assembly_flag_subs_.insert(
        make_pair(module_name, nh_.subscribe(module_name + string("/assembly_flag"), 1, &BeeNavigator::assemblyFlagCallback, this)));
    assembly_flags_.insert(std::make_pair(i + 1, false));
  }
}

void BeeNavigator::joyStickControl(const sensor_msgs::JoyConstPtr& joy_msg)
{
  ...
  {
    ROS_WARN("the joystick type is not supported (buttons: %d, axes: %d)", (int)joy_msg->buttons.size(),
  ...
  else if (joy_cmd.buttons[JOY_BUTTON_REAR_LEFT_1] == 1)
  {
    ...
    {
      target_roll_pitch.setY(getFinalTargetBaselinkRPY().y() + max_target_roll_pitch_rate_);
      joy_pitch_positive_flag_ = true;
      ROS_INFO_STREAM("Set target base link rot to [" << target_roll_pitch.x() << ", " << target_roll_pitch.y()
                                                       << ", " << target_roll_pitch.z() << "]");
    }
    ...
    {
      target_roll_pitch.setX(getFinalTargetBaselinkRPY().x() + max_target_roll_pitch_rate_);
...
void BeeNavigator::naviCallback(const aerial_robot_msgs::FlightNavConstPtr& msg)
{
  if (getNaviState() == TAKEOFF_STATE || BaseNavigator::getNaviState() == LAND_STATE || getModuleState() != SEPARATED)
    return;
  ...
  switch (msg->pos_xy_nav_mode)
  {
    case aerial_robot_msgs::FlightNav::POS_MODE:
    {
      ...
      if (msg->target == aerial_robot_msgs::FlightNav::BASELINK)
      {
        ...
        target_cog_pos -=
            tf::Matrix3x3(tf::createQuaternionFromYaw(getTargetRPY().z())) * cog2baselink_tf.getOrigin();
      }
      else if (msg->target == CONTACT_POINT)
      {
        tf::Transform cog2cp_tf;
        tf::transformKDLToTF(bee_robot_model_->getCog2Cp<KDL::Frame>(), cog2cp_tf);
  ...
  if (msg->pos_xy_nav_mode != aerial_robot_msgs::FlightNav::ACC_MODE)
    setTargetZeroAcc();
...
void BeeNavigator::assemblyNavCallback(const aerial_robot_msgs::FlightNavConstPtr& msg)
{
...
void BeeNavigator::setAssemblyFinalTargetBaselinkRotCallback(const spinal::DesireCoordConstPtr& msg)
{
  if (getModuleState() != SEPARATED)
    setFinalTargetBaselinkRPY(tf::Vector3(msg->roll, msg->pitch, msg->yaw));
}

void BeeNavigator::assemblyFlagCallback(const diagnostic_msgs::KeyValue& msg)
{
...
void BeeNavigator::rotateContactPointFrame()
{
  geometry_msgs::TransformStamped tf = bee_robot_model_->getContactFrame<geometry_msgs::TransformStamped>();
  tf.header.stamp = ros::Time::now();
  tf.header.frame_id = tf::resolve(std::string(nh_.getNamespace()), bee_robot_model_->getRootFrameName());
  tf.child_frame_id = tf::resolve(std::string(nh_.getNamespace()), std::string("contact_point"));
  br_.sendTransform(tf);
}

void BeeNavigator::calcCenterOfMoving()
{
  ...
  if (control_flag_)
  {
    float x = center_of_moving.x();
    float cog_com_x = (fabs(x) < 1e-6f) ? 0.0f : center_of_moving.norm() * x / fabs(x);
    Eigen::Vector3f cog_com_dist(cog_com_x, 0, 0);
...
void BeeNavigator::convertTargetPosFromCoG2CoM()
{
  tf::Transform cog2com_tf;
  tf::transformKDLToTF(getCog2CoM<KDL::Frame>(), cog2com_tf);
  tf::Matrix3x3 cog_orientation_tf;
  tf::matrixEigenToTF(bee_robot_model_->getCogDesireOrientation<Eigen::Matrix3d>(), cog_orientation_tf);
  tf::Vector3 com_conversion =
      cog_orientation_tf * tf::Matrix3x3(tf::createQuaternionFromYaw(getTargetRPY().z())) * cog2com_tf.getOrigin();
  bool current_assembled = getCurrentAssembled();
  bool reconfig_flag = getReconfigFlag();

  KDL::Frame empty_frame;

  if (pre_assembled_ && !current_assembled)
  {
  ...
  else if ((!pre_assembled_ && current_assembled) || (current_assembled && reconfig_flag))
  {
    int my_id = getMyID();
    ...
    ROS_INFO_STREAM("ID: " << my_id << "'s orientation_err is " << "(" << orientation_err.x() << ", "
                           << orientation_err.y() << ", " << orientation_err.z() << ")");
    tf::Matrix3x3 att_err_mat =
        tf::Matrix3x3(tf::createQuaternionFromRPY(orientation_err.x(), orientation_err.y(), orientation_err.z()));
  ...
  if (getNaviState() == HOVER_STATE || getNaviState() == TAKEOFF_STATE)
  {
    setTargetPosX(target_cog_pos.x());
    setTargetPosY(target_cog_pos.y());
    setTargetPosZ(target_cog_pos.z());
...
void BeeNavigator::rosParamInit()
{
  ...
  nh_.param("robot_id", my_id_, 0);
  nh_.param("aerial_robot_base_node/tf_prefix", my_name_, std::string());
  if (!my_name_.empty() && std::isdigit(static_cast<unsigned char>(my_name_.back())))
    my_name_.pop_back();
}

/* plugin registration */
#include <pluginlib/class_list_macros.h>
PLUGINLIB_EXPORT_CLASS(aerial_robot_navigation::BeeNavigator, aerial_robot_navigation::BaseNavigator);
